...
export const StepDetailPanel: React.FC<StepDetailPanelProps> = ({ step, className }) => {
  ...
  return (
    <div key={key} className={cn("w-full transition-all duration-500 ease-out", animate ? "opacity-0 translate-y-4" : "opacity-100 translate-y-0", className)}>
      <GlassPanel elevation="high" tint="light" className="p-8 md:p-12 w-full max-w-4xl mx-auto rounded-2xl flex flex-col gap-10 border border-gray-200 bg-white/50">
        
        {/* Header & Description */}
        <div>
          <div className="flex items-end gap-4 mb-6">
            <span className="text-6xl md:text-8xl font-black tracking-tighter text-gray-200 leading-none">
              {step.stepNumber}
            </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pt-8 border-t border-gray-200">
          ...
          <div>
            <div className="flex items-center gap-2 mb-6">
              <ListChecks className="text-yellow-500 w-5 h-5" />
            ...
            <ul className="flex flex-col gap-4">
              {step.deliverables.map((deliverable, idx) => (
                <li key={idx} className="flex items-start gap-3 bg-white p-3 rounded-lg border border-gray-100 shadow-sm">
                  ...
                  <span className="text-gray-900 font-semibold leading-relaxed">{deliverable}</span>
                </li>
              ))}
            </ul>
          </div>
